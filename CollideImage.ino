...
void loop() {
  ...
  Sprites::drawExternalMask(x1, y1, circle,   circle_mask,   0, 0);
  Sprites::drawExternalMask(x2, y2, triangle, triangle_mask, 0, 0);

  arduboy.drawPixel(x1, y1);
  arduboy.drawPixel(x1 + 31, y1);
  arduboy.drawPixel(x1, y1 + 31);
  arduboy.drawPixel(x1 + 31, y1 + 31);
  
  arduboy.drawPixel(x2, y2);
  arduboy.drawPixel(x2 + 27, y2);
  arduboy.drawPixel(x2, y2 + 27);
  arduboy.drawPixel(x2 + 27, y2 + 27);

  arduboy.display();

  if (checkCollision) { 
  
    if (collide (x1, y1, circle, x2, y2, triangle)) {
      flashRedLED();
    }
...
bool collide(int16_t x1, int16_t y1, uint8_t *img1, int16_t x2, int16_t y2, uint8_t *img2) {
  ...
  if (!(x2       >= x1 + w1 ||
    ...
    // Data is retrieved from the each image using a separate index. The index is calculated by
    // multiplying the starting row by the image width and adding the left column of the overlapping
    // area.  An additional two bytes are added to account for hte image height and width that prefix
    // image data.
